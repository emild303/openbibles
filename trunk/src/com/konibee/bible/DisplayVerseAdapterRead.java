...
public class DisplayVerseAdapterRead extends ArrayAdapter<DisplayVerse> implements OnCreateContextMenuListener{
	private List<DisplayVerse> verseList = new ArrayList<DisplayVerse>();
	private int currentFontSize;
	private Context context;

	public DisplayVerseAdapterRead(Context context, int textViewResourceId,
		...
		for (int i=0; i<posRFList.size(); i++) {
			...
			spanText.setSpan(new ClickableSpan() {
				...
				public void onClick(View widget) {
					ReadBible readBibleActivity = (ReadBible) context;
					TextView txtFootnote = (TextView) readBibleActivity.getFootnoteView().findViewById(R.id.txtFootnote);
					txtFootnote.setText(textRFList.get(j));
					readBibleActivity.getFootnoteDialog().show();
				}
